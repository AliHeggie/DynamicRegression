...
class SARIMAX(MLEModel):
    ...
    **kwargs
    ...
    def __init__(self, endog, exog=None,exog_models=False, order=(1, 0, 0),
        ...
        if isinstance(exog_models,MLEModel):
            self.exog_models = [exog_models]
            self.time_varying_regression = True
        elif isinstance(exog_models,list):
            self.exog_models = exog_models
            self.time_varying_regression = True
        else:
            self.exog_models = exog_models
        ...
        # If using model for exog coefficients we must use state regression
        if self.exog_models:
        ...
        # We should either recieve no exog_model, a single exog_model
        # or a list of k_exog exog_models
        if exog_models:
            if len(self.exog_models)!=self.k_exog and len(self.exog_models)!=1:
        ...
        if self.state_regression:
            if self.exog_models:
                self.k_exog_states = sum([mod.k_states for
                    mod in self.exog_models])
                self.k_exog_var = 0
                for mod in self.exog_models:
                    self.k_exog_var += len(set(mod.parameters.keys()) & set(mod.parameters_state_cov.keys()))
            else:
                self.k_exog_states = self.k_exog
            k_states += self.k_exog_states
    ...
    def initialize(self):
        ...
        if self.state_regression and self.time_varying_regression:
            ...
            if self.exog_models:
                self._exog_variance_idx = ('state_cov', idx[0][-self.k_exog_states:],
                                       idx[1][-self.k_exog_states:])
            else:
    ...
    def initial_design(self):
        ...
        # If we have exog_models then the design matrix for the exog_model
        # multiplied by the exogonous covariate becomes part of the time varying
        # design matrix
        if self.state_regression:
            if self._k_order > 0:
                if self.exog_models:
                    exog_design = np.hstack([self.exog[:,i]*
                        self.exog_models[i].ssm.design for 
                        i in range(self.exog.shape[1])])
                else:
                    exog_design = self.exog
                design = np.c_[
                    np.reshape(
    ...
    def initial_transition(self):
        ...
        if self.state_regression:
            if self.exog_models:
                exog_transition = [mod.ssm.transition[:,:,0] for mod in
                    self.exog_models]
                start = -self.k_exog_states
                transition[start:, start:] = scipy.linalg.block_diag(
                    *exog_transition)

                # Autoregressive component
                start = -(self.k_exog_states + self._k_order)
                end = -self.k_exog_states if self.k_exog > 0 else None
            else:
                start = -self.k_exog
                # T_\beta
    ...
    def initial_selection(self):
        ...
        if not (self.state_regression and self.time_varying_regression):
            ...
            if self.exog_models:
                for i in range(self.k_exog_states, 0, -1):
                    selection[-i, -i] = 1
            else:
    ...
    def start_params(self):
        ...
        if self.state_regression and self.time_varying_regression:
            ...
            if self.exog_models:
                params_exog_variance = [1] * self.k_exog_var
            else:
    ...
    def transform_params(self, unconstrained):
        ...
        # If using exog_model do not transform because the parameters
        # will be transformed when using the update method from the 
        # exog model
        if self.state_regression and self.time_varying_regression:
            if self.exog_models:
                end += self.k_exog_var
                start += self.k_exog_var 
            else:
                end += self.k_exog
    ...
    def update(self, params, transformed=True, complex_step=False):
        ...
        if self.k_posdef > 0:
            ...
            if self.state_regression and self.time_varying_regression:
                # If we have exog_models then call each exog_model's update
                # method and then use the resulting stat_covariace matrices
                # to update the state_covariance matrix
                if self.exog_models:
                    start = 0
                    end = 0
                    exog_cov = list()
                    for mod in self.exog_models:
                        ...
                        mod_cov_params = params_exog_variance[start:end]
                        params = np.zeros(mod.k_params)
                        idx = [list(mod.parameters.keys()).index(key) for key in list(mod.parameters_state_cov.keys())]
                        params[idx] = mod_cov_params
                        mod.update(params)
                        exog_cov = [mod.ssm.state_cov[:,:,0]]
                        start += mod.k_state_cov
                    self.ssm[self._exog_variance_idx] = np.diag(scipy.linalg.block_diag(*exog_cov))
